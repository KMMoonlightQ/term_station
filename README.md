...
**文字选择**：在终端内容区按住鼠标左键拖动可高亮选择文字，松开后保留选区，不自动复制；支持中文、跨行和反向选择。程序启用鼠标支持时，用 `Shift+拖动` 选择文字。拖动期间暂缓显示新输出，松开后恢复；`Ctrl+C` 仍发送给 Shell，用于中断命令。若宿主终端拦截了 Shift+鼠标，请使用其原生选择与复制操作。
